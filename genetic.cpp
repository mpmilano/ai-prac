...
auto genetic(F generate_f, int k, G rank_f, H done_f, I mutate_f){
	...
	function<bool (const vector<T>&)> done = done_f;
	function<vector<T> () > generate = generate_f;
	function<double (const vector<T>&) > rank = rank_f;
	...
	list<vector<T> > parents;
	for (int i = 0; i < k; ++i) parents.push_back(generate());
	std::function<const vector<T>& (const decltype(parents)&)> recr;
	recr = [&recr, &done, &rank, &mutate](const auto& parents) -> const vector<T>&{
		...
		list<pair<vector<T>, vector<T> > > couples;
